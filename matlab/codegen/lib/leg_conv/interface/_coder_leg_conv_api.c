...
static const mxArray *b_emlrt_marshallOut(const real32_T u[12]);
...
void leg_spd_api(const mxArray *const prhs[4], const mxArray **plhs)
{
  ...
  real32_T phi4;
  st.tls = emlrtRootTLSGlobal;
  spd = (real32_T(*)[2])mxMalloc(sizeof(real32_T[2]));
  /* Marshall function inputs */
  dphi1 = emlrt_marshallIn(&st, emlrtAliasP(prhs[0]), "dphi1");
  dphi4 = emlrt_marshallIn(&st, emlrtAliasP(prhs[1]), "dphi4");
  phi1 = emlrt_marshallIn(&st, emlrtAliasP(prhs[2]), "phi1");
  phi4 = emlrt_marshallIn(&st, emlrtAliasP(prhs[3]), "phi4");
  /* Invoke the target function */
...
void lqr_k_api(const mxArray *prhs, const mxArray **plhs)
{
  ...
  lqr_k(L0, *K);
  /* Marshall function outputs */
  *plhs = b_emlrt_marshallOut(*K);
}

/*
 * File trailer for _coder_leg_conv_api.c
 *
 * [EOF]
 */
